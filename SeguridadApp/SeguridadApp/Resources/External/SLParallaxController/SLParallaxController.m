...
#define OPEN_SHUTTER_LATITUDE_MINUS          .001
#define CLOSE_SHUTTER_LATITUDE_MINUS         .006
...
{
    MKPointAnnotation* single_annotation = [[MKPointAnnotation alloc] init];
    single_annotation.coordinate = self.selectedLocation;
    [self.mapView addAnnotation:single_annotation];
    [self zoomToLocation:self.selectedLocation minLatitude:self.latitudeUserUp animated:self.regionAnimated];
}
...
-(void)setupTableView{
    self.tableView                  = [[UITableView alloc]  initWithFrame: CGRectMake(0, 0.0, 320, self.heighTableView)];
    ...
    [self.tableView setAutoresizingMask:UIViewAutoresizingFlexibleBottomMargin|UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleWidth];
...
- (void)zoomToLocation:(CLLocationCoordinate2D)location minLatitude:(float)minLatitude animated:(BOOL)anim
{
    MKCoordinateRegion region;
    CLLocationCoordinate2D loc  = location;
    loc.latitude                = loc.latitude - minLatitude;
    region.center               = loc;
    region.span                 = MKCoordinateSpanMake(.02, .02);       //Zoom distance
...
-(void) openShutter{
    [UIView animateWithDuration:0.2
                     ...
                     completion:^(BOOL finished){
                         ...
                         [self zoomToLocation:self.selectedLocation minLatitude:self.latitudeUserDown animated:self.regionAnimated];
...
- (void)zoomToUserLocation:(MKUserLocation *)userLocation minLatitude:(float)minLatitude animated:(BOOL)anim
{
    ...
    CLLocationCoordinate2D loc  = self.selectedLocation;
    loc.latitude                = loc.latitude - minLatitude;
    region.center               = loc;
...
-(void)mapView:(MKMapView *)mapView didUpdateUserLocation:(MKUserLocation *)userLocation{
    if(_isShutterOpen)
        [self zoomToLocation:self.selectedLocation minLatitude:self.latitudeUserDown animated:self.userLocationUpdateAnimated];
    else
        [self zoomToLocation:self.selectedLocation minLatitude:self.latitudeUserUp animated:self.userLocationUpdateAnimated];
}
